...
public class RootServerSearch implements Runnable {
    Socket con;

    RootServerSearch(Socket s) {
        con = s;
    }
    ...
    public void run() {
        try {
            ...
            Scanner sc = new Scanner(new File("root_dns_table.txt"));
            while (sc.hasNext()) {
                ...
                if (arr.get(0).equals(site)) {
                    new HandleQuery(arr, con, "Root server");
                    return;
                }
            }
            Socket nxt = new Socket("localhost", 5050);
